...
function public_server_register_account($a,$b) {
	...
	if(! $days)
	$r = q("UPDATE user set expire = %d where uid = %d limit 1",
		intval($days_posts),
		intval($uid)
	);
    else
	...
	if ($flagpostsexpire) {
		...
		if(count($r)) {
			foreach($r as $rr) {
				...
				q("update `pconfig` set v=1 WHERE cat='expire' and k='items'");
				// set_pconfig($rr['uid'], 'expire', 'items', 1);
				// set_pconfig($rr['uid'], 'expire', 'notes', 1);
			}
		}
        }
